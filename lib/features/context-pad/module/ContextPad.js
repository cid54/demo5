import { every, forEach, isArray, isFunction } from "min-dash";
 ...
 */
export default function ContextPad(
  canvas,
  elementRegistry,
  eventBus,
  scheduler
...
ContextPad.prototype.trigger = function (action, event, autoActivate) {
  ...
  if (action === "mouseover") {
    this._timeout = setTimeout(function () {
      self._mouseout = self.triggerEntry(
        entry,
        "hover",
        originalEvent,
        autoActivate
 ...
 * @param {boolean} [autoActivate=false]
 */
ContextPad.prototype.triggerEntry = function (
  entryId,
  action,
  event,
  autoActivate
...
ContextPad.prototype.getPad = function (target) {
  console.warn(
    new Error(
      "ContextPad#getPad is deprecated and will be removed in future library versions, cf. https://github.com/bpmn-io/diagram-js/pull/888"
    )
  );
...
ContextPad.prototype._getPosition = function (target) {
  if (!isArray(target) && isConnection(target)) {
    ...
    return {
      ...
      top: y + CONTEXT_PAD_MARGIN + this._canvas.zoom(),
    };
  }
  ...
  return {
    left:
      ...
      CONTEXT_PAD_MARGIN * this._canvas.zoom(),
...
ContextPad.prototype._getTargetBounds = function (target) {
  ...
  return elementsGfx.reduce(
    function (bounds, elementGfx) {
...
function targetsEqual(target, otherTarget) {
  ...
  return (
    target.length === otherTarget.length &&
    every(target, function (element) {
