...
export default function DEMOLayouter() {}

inherits(DEMOLayouter, BaseLayouter);

DEMOLayouter.prototype.layoutConnection = function (connection, hints) {
  if (!hints) {
    hints = {};
  }
  ...
  if (is(connection, "demo:Flow")) {
    if (
      source.type !== "demo:InformationBank" &&
      target.type !== "demo:InformationBank" &&
      target.type !== "demo:DemoBoard"
    ) {
