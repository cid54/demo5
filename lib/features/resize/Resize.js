...
import { asTRBL, getMid, roundBounds } from "diagram-js/lib/layout/LayoutUtil";
...
export default function Resize(eventBus, rules, modeling, dragging) {
  ...
  function handleMove(context, delta) {
    ...
    context.delta = delta;
    if (shape.type === "demo:Transaction") {
      aspectRatio = 1;
    } else if (shape.type === "demo:InformationBank") {
      const rectWidth = Math.sqrt(0.5 * 0.5 * 2);
      const rectHeight = rectWidth + rectWidth * (2 / 3);
      aspectRatio = 1 / rectHeight;
    }
    newBounds = resizeBounds(shape, direction, delta, aspectRatio);
  ...
  function handleStart(context) {
    ...
    if (context.shape.type === "label") return;
...
Resize.prototype.computeMinResizeBox = function (context) {
  ...
  if (shape.type === "demo:InformationBank") {
    const rectWidth = Math.sqrt(0.5 * 0.5 * 2);
    const rectHeight = rectWidth + rectWidth * (2 / 3);
    const aspectRatio = 1 / rectHeight;
    minDimensions = {
      width: DEFAULT_MIN_WIDTH,
      height: DEFAULT_MIN_WIDTH * (1 / aspectRatio),
    };
  }
