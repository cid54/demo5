...
function findDisplayCandidate(definitions) {
  return find(definitions.rootElements, function (e) {
    return is(e, "demo:DemoBoard");
  });
}

export default function DemoTreeWalker(handler, translate) {
  ...
  function handleDefinitions(definitions, rootBoard) {
    ...
    if (rootBoard && rootBoards.indexOf(rootBoard) === -1) {
      throw new Error(translate("rootBoard not part of demo:Definitions"));
    }
    ...
    if (is(rootElement, "demo:DemoBoard")) {
      handleDemoBoard(rootElement, ctx);
    }
  ...
  function handleDemoBoard(board, context) {
