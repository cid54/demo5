...
import { importDemoDiagram } from "./import/Importer";

import DEMOModdle from "./moddle/lib";

/**
 * A base viewer for DEMO diagrams.
 *
...
/**
 * Parse and render a DEMO diagram.
 *
 ...
 *
 * @param {String} xml the DEMO xml
 * @param {ModdleElement<PostitRootBoard>|String} [rootBoard] DEMO board or id of board to render (if not provided, the first one will be rendered)
 *
 ...
 * @param {ModdleElement<DEMORootBoard>|String} [rootBoard] Postit board or id of board to render (if not provided, the first one will be rendered)
 *
 ...
 * You can use these events to hook into the life-cycle.
 *
 * @param {String|ModdleElement<DemoRootBoard>} [rootBoardOrId] id or the diagram to open
 *
...
BaseViewer.prototype.open = function (rootBoardOrId) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (typeof rootBoardOrId === "string") {
      ...
      if (!rootBord) {
        var err2 = new Error("DemoRootBoard <" + rootBoardOrId + "> not found");
    ...
    importDemoDiagram(self, definitions, rootBord)
      .then(function (result) {
        var warnings = result.warnings;
...
/**
 * Export the currently displayed DEMO diagram as
 * a DEMO XML document.
 *
 * ## Life-Cycle Events
 *
...
/**
 * Export the currently displayed DEMOM diagram as
 * an SVG image.
 *
 * ## Life-Cycle Events
 *
...
BaseViewer.prototype.clear = function () {
  ...
  // in the DemoTreeWalker (and assume none are given
  // on reimport)
...
BaseViewer.prototype._createContainer = function (options) {
  var container = domify('<div class="demojs-container"></div>');
...
BaseViewer.prototype._createModdle = function (options) {
  ...
  return new DEMOModdle(moddleOptions);
};
...
function checkValidationError(err) {
  // check if we can help the user by indicating wrong demom xml
  ...
  if (match) {
    err.message =
      ...
      match[1] +
      "> detected; " +
      "this may indicate an invalid Demo board file" +
      match[2];
  }
